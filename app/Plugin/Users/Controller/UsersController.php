...
private function checkifonline(){
	if ($this->Auth->user()){
...
protected function _setupAuth() {
	...
	$this->Auth->allow('add', 'reset', 'verify', 'logout', 'view', 'reset_password', 'login', 'resend_verification','dashboard');
...
public function index() {
	$this->set('title_for_layout',$this->title.'Users');
	if ($this->Auth->user()){
		if ($this->Auth->user('is_admin') == 1 && $this->Auth->user('role') == 'admin'){
			$this->Paginator->settings['User']['order'] = 'last_login desc';
			$this->set('users', $this->Paginator->paginate($this->modelClass));
		}
		else
		{
			$this->Session->setFlash('This page is only for administrators','info');
			return $this->redirect('/');
		}
	}
	else{
		$this->Session->setFlash('You must first login !','info');
...
public function change_descs(){
	if ($this->Auth->user()){
		if ($this->data['Desc']){

			if (strlen($this->data['Desc']['descr']) > 500){
				$this->Session->setFlash('Description too long! Max chararacters : 500','error');
				$this->set('desc',$this->data['Desc']['descr']);
				$this->redirect($this->referer());
			}
			$this->User->id = $this->Auth->user('id');
			$this->User->saveField('descr',$this->data['Desc']['descr']);
			$this->User->clear();

			$this->Session->setFlash('Description changed!','success');
			$this->redirect('/users/dashboard/'.$this->Auth->user('username'));
		}
		else{
			return $this->redirect('/');
		}
...
public function comments(){
	$username = $this->request->pass[0];
	$user = $this->User->findByUsername($username);
	if (!$user){
		$this->Session->setFlash('This user does not exist','error');
		return $this->redirect('/');
	}
	else{
		$this->commentsLoad($this->User->Comment->findAllByParent_user_idAndApprovedAndModel($user['User']['id'],1,'User',array(),'created desc'));
...
public function favorites(){
	...
	if (!$user){
		...
		$favorites = $this->User->Favorite->find('all',array('conditions'=>array('user_id'=>$user['User']['id']),'order'=>'created desc'));
		$walls = [];
		foreach ($favorites as $key => $value) {
			$walls[$value['Favorite']['wallpaper_id']] = $this->User->Wallpaper->findById($value['Favorite']['wallpaper_id']);
		}
		$this->set('q',$walls);
		$this->set('username',$username);
	}
...
public function admin_index() {
	...
	if ($this->Auth->user()){
		if ($this->Auth->user("role") != "admin"){
			$this->Session->setFlash("You must be an admin to access this page","info");
			$this->redirect("/");
		}
	}
	else{
...
public function admin_add() {
	/*if ($this->Auth->user("role") == "admin"){
		...
		$data["User"]["id"] = str_replace("-", "", String::uuid());
		if (!empty($data)) {
			$data[$this->modelClass]['tos'] = true;
			$data[$this->modelClass]['email_verified'] = true;

			if ($this->{$this->modelClass}->add($data)) {
				$this->Session->setFlash(__d('users', 'The User has been saved'),"success");
...
public function admin_edit($userId = null) {
	/*if ($this->Auth->user("role") == "admin"){
		try {
			...
			if ($result === true) {
				$this->Session->setFlash(__d('users', 'User saved'),"success");
				$this->redirect(array('action' => 'index'));
			} else {
...
public function admin_delete($userId = null) {
	/*if ($this->Auth->user("role") == "admin"){
		if ($this->{$this->modelClass}->delete($userId)) {
			$this->Session->setFlash(__d('users', 'User deleted'),"success");
		} else {
			$this->Session->setFlash(__d('users', 'Invalid User'),"info");
		}
...
public function add() {
	...
	if ($this->Auth->user()) {
		$this->Session->setFlash(__d('users', 'Vous êtez déjà inscrit et connecté!'),"info");
		$this->redirect('/');
	}

	if (!empty($this->request->data)) {
		$data = $this->request->data;
		$data['User']['ip'] = $this->request->clientIp();
		$user = $this->{$this->modelClass}->register($data);
		if ($user !== false) {
			$Event = new CakeEvent(
			...
			$this->Session->setFlash('Votre compte a été créé et vous êtes connecter!',"success");
			$this->redirect('/');
		} else {
			unset($this->request->data[$this->modelClass]['password']);
			unset($this->request->data[$this->modelClass]['temppassword']);
			$this->Session->setFlash(__d('users', 'Your account could not be created. Please, try again.'),"error");
		}
	}
}
...
public function login() {
	$this->set('title_for_layout',$this->title.'Login');
	$Event = new CakeEvent(
	...
	if ($this->request->is('post')) {
		if ($this->Auth->login()) {
			...
			$this->Session->setFlash(sprintf(__d('users', '%s you have successfully logged in'), $this->Auth->user($this->{$this->modelClass}->displayField)),"success");
			if (!empty($this->request->data)) {
			...
			//return $data[$this->modelClass]['return_to'];
			...
			$this->Session->setFlash("Invalid e-mail / password combination","error");
		}
	}
...
public function logout() {
	...
	$this->RememberMe->destroyCookie();
	$this->Session->setFlash(sprintf(__d('users', '%s you have successfully logged out'), $user[$this->{$this->modelClass}->displayField]),"success");
	...
	if ($this->request->is('post')) {
		try {
			if ($this->{$this->modelClass}->checkEmailVerification($this->request->data)) {
				$this->_sendVerificationEmail($this->{$this->modelClass}->data);
				$this->Session->setFlash(__d('users', 'The email was resent. Please check your inbox.'),"success");
				$this->redirect('login');
			} else {
				$this->Session->setFlash(__d('users', 'Something went wrong with the email. Please try again.'),"error");
			}
		} catch (Exception $e) {
			$this->Session->setFlash($e->getMessage(),"error");
		}
...
public function verify($type = 'email', $token = null) {
	...
	try {
		...
		$this->Session->setFlash(__d('users', 'Your e-mail has been validated!'),"success");
...
public function request_new_password($token = null) {
	...
	if (!$data) {
		$this->Session->setFlash(__d('users', 'The url you accessed is not longer valid'),"error");
		return $this->redirect('/');
	}

	if ($this->{$this->modelClass}->save($data, array('validate' => false))) {
		$this->_sendNewPassword($data);
		$this->Session->setFlash(__d('users', 'Your password was sent to your registered email account'),"success");
		$this->redirect(array('action' => 'login'));
	}

	$this->Session->setFlash(__d('users', 'There was an error verifying your account. Please check the email you were sent, and retry the verification link.'),"error");
	$this->redirect('/');
}
...
public function reset_password($token = null, $user = null) {
	$this->set('title_for_layout',$this->title.'Reset password');
	if (empty($token)) {
		$admin = false;
		if ($user) {
	...
	if (!empty($this->request->data)) {
		...
		if (!empty($user)) {
			...
			if ($admin) {
				...
				$this->Session->setFlash(__d('users', 'You should receive an email with further instructions shortly'),"success");
				$this->redirect(array('action' => 'login'));
			}
		} else {
			$this->Session->setFlash(__d('users', 'No user was found with that email.'),"error");
			$this->redirect($this->referer('/'));
		}
	}
...
protected function _resetPassword($token) {
	...
	if (empty($user)) {
		$this->Session->setFlash(__d('users', 'Invalid password reset token, try again.'),"error");
	...
	if (!empty($this->request->data) && $this->{$this->modelClass}->resetPassword(Hash::merge($user, $this->request->data))) {
		if ($this->RememberMe->cookieIsSet()) {
			...
			$this->Session->setFlash(__d('users', 'Password changed, you can now login with your new password.'),"success");
